/**
 * The demo content, in one place.
 *
 * Read by the in-memory mock store (the default whenever DATABASE_URL is unset)
 * and by scripts/db-seed.mts when seeding a real Postgres, so the two can never
 * drift apart.
 */
